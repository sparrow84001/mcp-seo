...
export interface WebMcpDiagnosticIssue {
  checkName: string;
...
export interface WebMcpLanguageBlueprint {
  language: string;
  framework: string;
  packageName: string;
  fileLocation: string;
  transportType: 'Streamable HTTP' | 'HTTP + SSE' | 'In-Browser DOM';
  officialSpecRef: string;
  codeSnippet: string;
...
export interface SecurityHeadersAnalysis {
  isHttps: boolean;
  hsts: { present: boolean; value?: string; isValid: boolean; recommendations?: string };
  contentSecurityPolicy: { present: boolean; value?: string; recommendations?: string };
  xFrameOptions: { present: boolean; value?: string; isValid: boolean; recommendations?: string };
  xContentTypeOptions: { present: boolean; value?: string; isValid: boolean; recommendations?: string };
  referrerPolicy: { present: boolean; value?: string; isValid: boolean; recommendations?: string };
  permissionsPolicy: { present: boolean; value?: string; recommendations?: string };
  score: number; // 0-100
  grade: 'A+' | 'A' | 'B' | 'C' | 'D' | 'F';
  issues: AuditIssue[];
}
